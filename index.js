 ...
 let resultHeaderIp = document.getElementById('resultHeaderIP');
 let resultImg = document.getElementById('resultImg');
 let askImg = document.getElementById('ask-img');
 ...
 const setUI = async () => {
     ...
     // Answer is no
         ...
         resultHeaderIp.innerHTML = "IP: " + dbData.lastAnswerIP + " - " + dbData.lastAnswerIPProvider;
         lastAnswerDateText.innerHTML = `${uiData.lastAnswerHeading} ${dbData.lastAnswerDate}`;
     ...
     // No valid answer (yes / no) available
         ...
         askImg.style.display = "unset";
         askImg.src = uiData.askImgLink;
     }
 };
 setUI();
 ...
 const getIP = async () => {
     try {
         ...
         const jsonObj = (res.json()).then(async (d) => {
             dbData.lastAnswerIP = d.ip;
     ...
     try {
         const res1 = await fetch(`http://ip-api.com/json/${dbData.lastAnswerIP}`);
         const jsonObj1 = (res1.json()).then((e) => {
             dbData.lastAnswerIPProvider = e.isp;
         });
     } catch (err) {
         console.error(err);
     }
 ...
 const setFirebaseDataToLocalObj = () => {
     try {
         const ref = this.db.collection('valentines').doc(firestore.docID).set(dbData, {
             merge: true
         });
